...
uses
  ...
  TATransformations, TADbSource, TAFuncSeries, FilterIIRLPChebyshev, Math, types;
...
type
  ...
  TForm1 = class(TForm)
    ...
    FloatSpinEditRipple: TFloatSpinEdit;
    ...
    procedure FloatSpinEditRippleChange(Sender: TObject);
...
begin
  ...
  Application.Title:= 'Filter Low Pass Chebyshev Example' ;

  //Setup parameters

  rbSourceClick(nil);
  FloatSpinEditRippleChange(nil);
...
begin
    FilterIIRLPChebyshev1.FreqCut1:= FloatSpinEditFreqCut1.Value;
end;
...
begin
     FilterIIRLPChebyshev1.Gain:= FloatSpinEditGain.Value;
end;

procedure TForm1.FloatSpinEditRippleChange(Sender: TObject);
begin
    FilterIIRLPChebyshev1.Ripple:= FloatSpinEditRipple.Value / 10;
end;

procedure TForm1.FloatSpinEditSampleRateChange(Sender: TObject);
begin
  Timer1.Enabled:=false;
  FilterIIRLPChebyshev1.SampleRate:=FloatSpinEditSampleRate.Value;
  Series1.Clear;
  Series2.Clear;
  Timer1.Enabled:=true;
end;
...
begin
    FilterIIRLPChebyshev1.Order:= SpinEditOrder.Value;
end;
...
begin
  ...
  for I:= 1 to 5 do
  begin
    T:= Timer1.Tag / FilterIIRLPChebyshev1.SampleRate;
    ...
    Tmp := FilterIIRLPChebyshev1.Filter(V);
