...
uses
  ...
  TATransformations, TADbSource, TALegendPanel, types, FilterIIRLPButterworth,
  Math, UType, UConst, FilterIIRBSBessel, TACustomSeries;
...
begin
  ...
  Application.Title:= 'Filter Low Pass Butterworth Example';
...
begin
  FilterIIRLPButterworth1.Gain:= FloatSpinEditGain.Value;
end;

procedure TForm1.FloatSpinEditSampleRateChange(Sender: TObject);
begin
  Timer1.Enabled:=false;
  FilterIIRLPButterworth1.SampleRate:=FloatSpinEditSampleRate.Value;
  Series1.Clear;
  Series2.Clear;
  Timer1.Enabled:=true;
end;
...
begin
    FilterIIRLPButterworth1.FreqCut1:= FloatSpinEditFreqCut1.Value;
end;
...
begin
   FilterIIRLPButterworth1.Order:= SpinEditOrder.Value;
   SpinEditOrder.Value := FilterIIRLPButterworth1.Order; //Verificando visualmente el valor que realmente tomo la propiedad orden del filtro
end;
...
begin
  for I:= 1 to 5 do
  begin
    T:= Timer1.Tag / FilterIIRLPButterworth1.SampleRate;
    ...
    Series2.AddXY(T, FilterIIRLPButterworth1.Filter(V), '', clRed);
