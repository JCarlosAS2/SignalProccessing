...
uses
  ...
  Menus, Math, 
  Filter, FilterIIR, UType, Spin, FilterIIRHP, FilterIIRHPBessel;
...
begin
  FilterIIRHPBessel1.Order:= SpinEditOder.Value;
end;

procedure TfmMain.SpinEditFreqCut1Change(Sender: TObject);
begin
  FilterIIRHPBessel1.FreqCut1:= SpinEditFreqCut1.Value;
end;
...
begin
   FilterIIRHPBessel1.Gain:= SpinEditGain.Value;
end;

procedure TfmMain.SpinEditSampleRateChange(Sender: TObject);
begin
  Timer1.Enabled:=false;
  FilterIIRHPBessel1.SampleRate:=SpinEditSampleRate.Value;
  Series1.Clear;
  Series2.Clear;
  Timer1.Enabled:=true;
end;
